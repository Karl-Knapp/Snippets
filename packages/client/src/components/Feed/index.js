  ...
  useEffect(() => {
    const filterPosts = () => {
      if (searchInput === "") {
        ...
        let filtered = fuse.search(searchInput); //Non-fuzzy: posts.filter(post => post.text.toLowerCase().includes(searchInput.toLowerCase()))
        setFilteredPosts(filtered.map(filtered => filtered.item));
      }
    }
    filterPosts();
  }, [searchInput, posts])
